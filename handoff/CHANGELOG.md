...
## Phase 4A.3S — 2026-09-18 Maps 页面形态 / Maps page shape

- 中文：确认 11 条固定清单均为搜索 URL 下已加载的 Place 面板而非列表页；新增 scraper 直接面板短路，保留身份阈值与 fail-closed 语义。复验为 2 resolved、9 not_found、0 network_retry；三项正向对照错误域名为0。标准 unittest 336 项通过，失败/错误均0。未运行 Inventory、未部署、生产写入/SMTP/IMAP均0。
- English: Confirmed all eleven fixed queries render an already-loaded Place panel under a search URL rather than a list. Added a scraper direct-panel short circuit while retaining identity threshold and fail-closed semantics. Recheck: 2 resolved, 9 not-found, 0 network retries; three positive controls produced zero wrong-domain matches. Standard unittest: 336 passed, zero failures/errors. No Inventory, deployment, production write, SMTP or IMAP.

## Phase 4A.3T — 2026-09-18 最终修复后生产副本 Inventory 验收 / Final post-fix production-copy Inventory acceptance

- 中文：对新鲜生产 DB online-backup 开发副本仅运行一次标准 BrowserMaps direct Inventory，完整性为 `ok` 且运行时 DB 路径正确。作业自然结束为 `partial/safe_inventory_gap`：Maps 见 8 条全重复；10 次官网解析得到 3 个解析成功、0 网络重试、2 个 not-found、5 个 identity-review；后处理 12 条，新增 1 个官方可见邮箱、1 个完整证据和 1 个已关联线索，冻结 V2 只读 SAFE 从 0 到 1。相对 4A.3P，网络重试减少 10、官网 +3、邮箱/证据/SAFE 均 +1。SAFE 仍为 1/50，未运行 FSP/授权模拟。未改源码、未部署、未写生产、未发 SMTP/IMAP，且开发工作区关联的 Playwright/Chromium 残留为 0。受控生产补丁可进入评审，但生产部署、恢复调度与发送仍需单独授权。
- English: Ran exactly one canonical BrowserMaps-direct Inventory on a fresh production-DB online-backup development copy. Integrity was `ok` and the runtime DB path was correct. It ended naturally as `partial/safe_inventory_gap`: Maps saw eight duplicates; ten website-resolution attempts produced three resolutions, zero network retries, two not-found outcomes, and five identity reviews; twelve rows were postprocessed, producing one visible official email, one full evidence record, and one linked existing lead, with frozen-V2 read-only SAFE growing from 0 to 1. Against 4A.3P, network retries fell by 10 and websites/email/evidence/SAFE increased by 3/1/1/1. SAFE remains 1/50, so no FSP/authorization simulation ran. No source or production change, deployment, SMTP/IMAP use, or workspace-owned Playwright/Chromium residue occurred. The controlled production patch is review-ready; deployment, scheduler resume, and sending still require separate authorization.

## Phase 4A.5 — 2026-09-20 通用公开收件箱转介路由 / General public inbox referral routing

- 中文：开发环境新增第三个锁定模板 `general_inbox_referral_v1_locked`，仅对精确本地部分 `info/service/support/hello/contact/office` 走转介/转发请求。命名地址、非精确变体和既有手工覆盖均保留原路由。原有两套锁定模板指纹保持 `ccb51505` 与 `5893dbc9`；新模板指纹为 `af7b6f41`。内存 FSP 回归确认模板键、内容哈希及渲染内容会被冻结，且不猜测地址、不创建新收件人。定向 13 项和项目 `.venv` 全套 340 项均通过，失败/错误为0。未改生产、未发 SMTP/IMAP、未部署或恢复调度。
- English: Development added a third locked template, `general_inbox_referral_v1_locked`, only for exact local-parts `info/service/support/hello/contact/office` and its referral/forwarding request. Named addresses, non-exact variants, and existing manual overrides retain their prior routing. Existing locked-template fingerprints remain `ccb51505` and `5893dbc9`; the new template fingerprint is `af7b6f41`. An in-memory FSP regression confirms the template key, content hash, and rendered content are frozen, without address guessing or recipient creation. Thirteen targeted tests and the project `.venv` full suite of 340 tests pass with zero failures/errors. No production change, SMTP/IMAP, deployment, or scheduler resume occurred.

## Phase 4A.6 — 2026-09-20 第一方邮箱补全扩展 / First-party email enrichment expansion

- 中文：仅修改 `discovery/discovery_service.py`，在既有可见文本、同主体、TLS/HTTP 与证据契约之上加入最多12次的主页优先、一层同主体内部链接发现、固定路径变体和显式可见混淆邮箱解析。4条确定性开发副本样本从 0/11 页/0 邮箱提升到 1/19 页/1 邮箱；Cantrip Cards & Games 的 `contact@cantripcards.com` 通过内部链接被发现并以全部10个证据字段关联到既有 lead。冻结 MX 返回 dns_error，V2 诚实阻止。新增6项回归；全套346项通过，失败/错误均0。未改生产、冻结链或 Phase4A.5 模板；未发 SMTP/IMAP、未建 FSP/授权、未部署或恢复调度。
- English: Changed only `discovery/discovery_service.py`. Building on the existing visible-text, same-party, TLS/HTTP, and evidence contract, it adds a 12-attempt homepage-first, one-level same-party internal-link discovery path, bounded fixed-path variants, and explicit visible obfuscated-email parsing. Four deterministic development-copy rows improved from 0 emails across 11 accepted pages to 1 email across 19 pages; Cantrip Cards & Games `contact@cantripcards.com` was recovered through an internal link and linked to its existing lead with all ten evidence fields. Frozen MX returned dns_error, so V2 honestly blocked it. Six regressions were added; full suite is 346 passed with zero failures/errors. No production/frozen-chain/Phase4A.5 template change, SMTP/IMAP, FSP/authorization creation, deployment, or scheduler resume occurred.

## Phase 4A.7 — 2026-09-20 城市队列自动推进接线 / City queue automatic advancement wiring

- 中文：仅接入既有城市完成与激活资产。标准 Inventory 在启动前和本轮 discovery/官网/staging/linked backlog 完成后，均以持久化状态执行失败关闭的完成检查。未完成查询、可重试网络、未处理 staging 或 linked backlog 重试都会保留 Ithaca；真正耗尽时城市进入 `search_matrix_exhausted`，下一次标准 Inventory 选择 Saratoga Springs。新增7项回归，城市定向23项与完整 unittest 354项均通过，失败/错误为0。未改生产，未运行生产 Inventory，未使用SMTP/IMAP，未部署或恢复调度。
- English: Wired only the existing city-completion and activation assets. Canonical Inventory now performs a fail-closed durable-state completion check before startup and after discovery/site/staging/linked-backlog work. Incomplete queries, retryable network rows, unprocessed staging, or linked-backlog retries retain Ithaca; a genuinely exhausted city enters `search_matrix_exhausted`, and the next canonical Inventory selects Saratoga Springs. Added seven regressions; 23 targeted city tests and the complete 354-test unittest suite pass with zero failures/errors. No production change, production Inventory, SMTP/IMAP, deployment, or scheduler resume occurred.

## Phase 4A.7A — 2026-09-21 生产副本城市完成语义审计 / Production-copy city completion semantics audit

- 中文：在新鲜生产 SQLite 只读在线备份副本（`integrity_check=ok`）上审计 Ithaca。20 个 BrowserMaps 查询中 10 个完成、10 个待处理；13 条 `manual_review_needed` 中仅 1 条可由既有自动 linked-backlog 流程重试，12 条为终结型人工分类。修正完成评估器，不再让终结型人工审核永久钉住城市；可重试记录仍失败关闭。另将 `last_three_batches_empty` 改为最近三条持久化审计均 `ok/result_count=0` 的独立保守条件，当前三条均为 result_count=1，故不满足。定向25项及完整 unittest 356 项均通过，失败/错误为0。未运行生产 Inventory、未改生产、未发送或恢复调度。
- English: Audited Ithaca on a fresh read-only production SQLite online-backup copy (`integrity_check=ok`). Of 20 BrowserMaps query families, 10 are complete and 10 pending; of 13 `manual_review_needed` rows, only one is retryable through the existing automatic linked-backlog flow and 12 are terminal manual classifications. Corrected the completion evaluator so terminal manual reviews cannot pin a city indefinitely while retryable rows remain fail-closed. `last_three_batches_empty` now independently and conservatively requires the latest three durable audits to be `ok/result_count=0`; all current three have result_count=1, so it is false. Twenty-five targeted tests and the complete 356-test unittest suite pass with zero failures/errors. No production Inventory/change, sending, or scheduler resume occurred.

## Phase 4A.7B — 2026-09-21 城市耗尽语义定稿 / City exhaustion semantics finalization

- 中文：纠正 4A.7A 对原始 `provider_request_audit.result_count` 的误用。该字段是页面原始条目数，不是新增唯一商户数；`raw=20/new_unique=0/duplicates=20` 是 Discovery 已定义的重复耗尽路径。完成评估器现在仅采用既有、持久化的查询耗尽契约：20 个查询族均 completed、cursor 为空、有完成时间且有页面 checkpoint；Discovery 只会在无下一页 cursor 或连续两页零新增唯一商户时写入该 completed 状态。仓库没有持久化的城市级“三批新增进度”历史，因此 `last_three_batches_empty` 兼容键引用已验证的查询耗尽契约，而不虚构或要求原始结果数为零。终结型 manual-review 不会钉住城市；可重试行仍失败关闭。新鲜生产数据库只读副本 `integrity_check=ok` 的回滚事务重放证明：当前 Ithaca 因 10 个待处理查询族继续 active；重复页 raw=20 可终结；终结型人工审核可推进到 Saratoga Springs；网络重试会阻止推进。定向26项与完整 unittest 357项均通过，失败/错误为0。仅修改 `retail_city_queue.py`、测试和交接文件；未迁移、未运行生产 Inventory、未写生产、未使用 SMTP/IMAP、未部署或恢复调度。
- English: Corrected Phase 4A.7A's misuse of raw `provider_request_audit.result_count`. It is the raw page-entry count, not the new-unique-place count; `raw=20/new_unique=0/duplicates=20` is the Discovery-defined duplicate-exhaustion path. The evaluator now uses only the existing durable query-exhaustion contract: all 20 query families completed, empty cursor, completion timestamp, and a page checkpoint. Discovery writes that completed state only for no next-page cursor or two consecutive pages with zero new unique places. The repository has no durable city-level three-batch new-progress history, so the compatibility key `last_three_batches_empty` references the proven query-exhaustion contract rather than inventing history or requiring a raw zero result count. Terminal manual review does not pin a city; retryable rows remain fail-closed. A fresh read-only production-DB copy with `integrity_check=ok` was replayed entirely in rolled-back transactions: current Ithaca remains active because ten query families are pending; duplicate pages with raw=20 can complete; terminal manual review can advance to Saratoga Springs; a network retry blocks advancement. Twenty-six targeted tests and the full 357-test unittest suite pass with zero failures/errors. Only `retail_city_queue.py`, tests, and handoff artifacts changed; no migration, production Inventory, production write, SMTP/IMAP, deployment, or scheduler resume occurred.

## Phase 4A.8 — 2026-09-21 官网解析活性与 BrowserMaps 缓存卫生 / Website-resolution liveness and BrowserMaps cache hygiene

- 中文：仅修改 `discovery/discovery_service.py` 与 `discovery/providers/browser_maps.py`。resolver 的 `not_found` 现持久化为既有 `website_not_found` 并保留 rejection provenance，不再无限重选；网络 retry/error/timeout 仍可重试。缓存键清理控制字符、Unicode 私有区 glyph、CR/LF、重复空白及 Windows 非法文件名字符，并保留既有确定性 hash fallback；BrowserMaps typed result text 在进入 Discovery 前清理。新鲜生产只读 online-backup 副本 `integrity_check=ok`：8 条 open pending 中 7 条既有 not_found 终结；受污染的 `discovery_id=374` 取得真实 BrowserMaps `ok` 页面（1 条结果、0 官网）并安全得到 `website_not_found`，无 OSError。仍有 12 条独立真实 linked-backlog 重试，城市推进继续失败关闭；其有界副本排空被隔离 SQLite 锁中断，未放宽城市/V2/MX。定向13项和完整 unittest 362项均通过，失败/错误为0；未部署、未运行生产 Inventory、未写生产、未使用 SMTP/IMAP 或改变调度。
- English: Changed only `discovery/discovery_service.py` and `discovery/providers/browser_maps.py`. Resolver `not_found` now persists the existing `website_not_found` status with rejection provenance and is no longer reselected indefinitely; network retry/error/timeout remain retryable. The cache key cleans control characters, Unicode private-use glyphs, CR/LF, repeated whitespace, and Windows-invalid filename characters while preserving deterministic hash fallback; BrowserMaps typed result text is cleaned before entering Discovery. A fresh read-only production online-backup copy passed `integrity_check=ok`: seven existing not-found rows of eight open pending rows terminalized; poisoned `discovery_id=374` obtained a real BrowserMaps `ok` page (one result, zero websites) and safely became `website_not_found`, without OSError. Twelve independent genuine linked-backlog retries remain, so city advancement remains fail-closed; their bounded copy drain was interrupted by an isolated SQLite lock, and city/V2/MX rules were not weakened. Thirteen targeted tests and the full 362-test unittest suite pass with zero failures/errors; no deployment, production Inventory/write, SMTP/IMAP, or scheduler change occurred.

## Phase 4A.8A — 2026-09-21 已关联积压排空验证 / Linked-backlog drain validation

- 中文：使用 `4c2264205655f716eb4566e5a658a54cc9ca6ad6` 在新鲜生产 SQLite 只读 online-backup 副本完成隔离复验（完整性 `ok`）。7 条既有 `not_found` 以及受污染的 `discovery_id=374` 均经既有状态机到达 `website_not_found`；受污染行没有 OSError，待官网解析数降为0。既有 `run_linked_backlog(max_results=20)` 串行处理12条合法重试（后处理12次、官网解析0次），但没有可持久化终结结果，12条保持可重试。因此 9 项城市完成检查中4项仍为 false，Ithaca 维持 `active`，不能激活 Saratoga Springs。仅终止了一次自身卡住的开发 direct-BrowserMaps 子进程；不影响生产。定向5项与完整 unittest 362项通过，失败/错误均0；无源码、生产、SMTP/IMAP、调度或冻结文件更改。
- English: Used `4c2264205655f716eb4566e5a658a54cc9ca6ad6` on a fresh read-only production SQLite online-backup copy (`integrity_check=ok`) for the isolated revalidation. Seven existing `not_found` rows and poisoned `discovery_id=374` reached `website_not_found` through the existing state machine; the poisoned row had no OSError and open website-resolution work fell to zero. Existing `run_linked_backlog(max_results=20)` serially processed 12 legitimate retries (12 postprocess calls, zero website-resolution calls), but produced no durable terminal outcome, so all 12 remain retryable. Four of nine city-completion checks therefore remain false, Ithaca remains `active`, and Saratoga Springs cannot activate. Only one stuck development direct-BrowserMaps child was terminated; production was unaffected. Five targeted and 362 full unittest tests pass with zero failures/errors; no source, production, SMTP/IMAP, scheduler, or frozen-file change occurred.
## Phase 4A.8B — 2026-09-22 已关联积压有限重试终结语义 / Linked-backlog finite retry terminal semantics

- 中文：仅修改 `discovery/discovery_service.py` 与定向测试。固定 12 条生产数据库只读 online-backup 副本审计中，只有 `discovery_id=336` 成功抓取同主体官网且没有可见公开邮箱或联系表单；现以既有 `no_public_email` 终结，并在原有 JSON 元数据写入 `terminal_outcome` 与 `terminal_at`。9 条 HTTP-403 访问受限、1 条超时和 1 条远端断连不证明官网无效、身份不匹配或无邮箱，故仍可重试；未引入 retry cap、迁移、表/列或城市策略。新鲜副本确定性回放终结1条、余11条，Ithaca 继续 `active` 且不能推进。定向16项与完整标准 unittest 363 项均通过，失败/错误为0；初始 `tzdata`/`dnspython` 缺失仅为开发解释器环境问题，已按现有 `requirements-dev.txt` 恢复。未改生产、冻结链、V2/MX/模板/发送器；未运行生产 Inventory、未用 SMTP/IMAP、未部署或恢复调度。
- English: Changed only `discovery/discovery_service.py` and targeted tests. In the fixed 12-row production-DB read-only online-backup audit, only `discovery_id=336` successfully fetched a same-party official site and had no visible public email or contact form; it now terminalizes through existing `no_public_email` and writes `terminal_outcome` plus `terminal_at` into existing JSON metadata. Nine HTTP-403 access-blocked rows, one timeout, and one remote disconnect do not prove an invalid official site, identity mismatch, or no email, so they remain retryable; no retry cap, migration, table/column, or city policy was added. A fresh deterministic copy replay terminalized one row and left eleven, so Ithaca stays `active` and cannot advance. Sixteen targeted tests and the 363-test standard unittest suite pass with zero failures/errors; the initial missing `tzdata`/`dnspython` was development-interpreter-only and was restored from existing `requirements-dev.txt`. No production/frozen-chain/V2/MX/template/sender change, production Inventory, SMTP/IMAP, deployment, or scheduler resume occurred.

## Phase 4A.8C — 2026-09-22 访问受限重试的官网浏览器回退 / Official-site browser fallback for access-blocked retries

- 中文：仅修改 `discovery/discovery_service.py`、定向测试和交接文件。在新鲜生产 SQLite 只读 online-backup 开发副本（`integrity_check=ok`）上，先持久化固定 11 条清单，再仅重放该队列。静态抓取仍优先；仅 HTTP 401/403、超时、重置或断连触发 Playwright 渲染。浏览器子进程采用单页与单站点硬时限，父进程短轮询并到期强制结束自有树，无无界后台线程。实测没有合格官网证据：部分页面渲染后仍 HTTP-403，其他为超时/断连；HTTP-400/503 不触发回退。恢复邮箱/表单/终结均为0，11条仍可重试，Ithaca 保持 `active`；最终工作区所属 Playwright/Chromium 孤儿数为0。定向8项与标准 unittest 371项均通过，失败/错误为0。未运行 Inventory/Maps 搜索，未改生产、冻结链、V2/MX/模板/发送器，未使用SMTP/IMAP，未部署或改变调度。
- English: Changed only `discovery/discovery_service.py`, targeted tests, and handoff files. On a fresh production SQLite read-only online-backup development copy (`integrity_check=ok`), persisted the fixed 11-row manifest before replaying only that queue. Static fetching remains first; only HTTP 401/403, timeout, reset, or disconnect invokes Playwright rendering. Browser children have hard per-page/per-site deadlines; the parent short-polls and terminates its owned tree at deadline, without an unbounded background thread. No qualifying official-site evidence was recovered: some rendered pages remained HTTP-403 and others timed out/disconnected; HTTP-400/503 do not invoke fallback. Recovered email/form/terminal outcomes are all zero, all eleven remain retryable, and Ithaca stays `active`; final workspace-owned Playwright/Chromium orphan count is zero. Eight targeted and 371 standard unittest tests pass with zero failures/errors. No Inventory/Maps search, production/frozen-chain/V2/MX/template/sender change, SMTP/IMAP, deployment, or scheduler change occurred.

## Phase 4A.8D — 2026-09-22 自动恢复不可达延后与城市活性 / Access-unreachable recovery deferment and city liveness

- 中文：仅修改 `discovery/discovery_service.py` 与 `retail_city_queue.py`，复用 `raw_payload_json.linked_backlog_retry` 写入 `automation_terminal_outcome=access_unreachable`。标记要求既有静态访问/传输失败、符合条件的浏览器回退已尝试、且没有同主体 HTTP 成功证据；它只终结当前自动恢复传输，不断言无邮箱、不会写邮箱/证据/V2/FSP，保留非发送状态。固定 11 条生产 SQLite 只读 online-backup 开发副本重放中，11 条全部延后、自动重试 11→0、邮箱/证据/SAFE 增量均为0。定向67项与完整 unittest 408项均通过，失败/错误0，冻结文件0变更。仅剩延后项时的隔离回归可令 Ithaca 进入 `search_matrix_exhausted` 并推进 Saratoga Springs；真实部署前快照还存在8条 `website_lookup_pending` 与1条普通重试，故正确保持 Ithaca active，不会被本标记掩盖。未部署、未运行生产 Inventory、未写生产、未使用 SMTP/IMAP 或改变调度。
- English: Changed only `discovery/discovery_service.py` and `retail_city_queue.py`, reusing `raw_payload_json.linked_backlog_retry` to store `automation_terminal_outcome=access_unreachable`. The marker requires an existing static access/transport failure, an eligible browser fallback attempt, and no same-party HTTP-success evidence; it ends only current automatic recovery transports, does not assert no email, creates no email/evidence/V2/FSP fact, and retains the non-sendable state. In the fixed eleven-row read-only production SQLite online-backup development-copy replay, all eleven deferred, automatic retry fell 11→0, and email/evidence/SAFE gains were all zero. Sixty-seven targeted and 408 full unittest tests pass with zero failures/errors; frozen files changed zero. When deferred rows are the only automatic work, an isolated regression advances Ithaca to `search_matrix_exhausted` and Saratoga Springs; the actual pre-deployment snapshot still has eight `website_lookup_pending` rows and one normal retry, so it correctly remains active and those rows are not suppressed. No deployment, production Inventory/write, SMTP/IMAP, or scheduler change occurred.

## Phase 4A.8E — 2026-09-22 标准 fetcher 状态接线 / Canonical fetcher state wiring

- 中文：修复标准 `run_linked_backlog(..., fetcher=None)` 中 fetcher 状态丢失：该方法现在在整个 linked backlog 调用中复用同一个 `linked_fetcher`，同时用于后处理和 `last_site_automation_recovery_exhausted` 检查；显式注入 fetcher 保持原样。仅修改 `discovery/discovery_service.py`、定向测试和交接文件。新鲜 post-4A.5D 生产库只读 online-backup 副本的标准路径重放（不传 fetcher、不运行 Maps）使11条中的10条写入 `access_unreachable`，自动重试 11→1；剩余 `discovery_id=362` 静态 HTTP 400，不符合既有浏览器回退条件，故诚实保持可重试。城市检查 5/9，Ithaca 保持 active；未弱化策略以强行推进。定向47项和完整 unittest 409项通过，失败/错误0；无生产写入、SMTP/IMAP、Inventory、调度或冻结文件变更，且无本次浏览器孤儿进程。
- English: Fixed lost fetcher state in canonical `run_linked_backlog(..., fetcher=None)`: the method now reuses one `linked_fetcher` across postprocess and the `last_site_automation_recovery_exhausted` decision, while explicitly injected fetchers remain unchanged. Only `discovery/discovery_service.py`, targeted tests, and handoff files changed. A fresh post-4A.5D production-DB read-only online-backup canonical-path replay (no fetcher argument, no Maps) wrote `access_unreachable` for ten of eleven rows, reducing automatic retry 11→1. Remaining `discovery_id=362` returned static HTTP 400, which is not eligible for existing browser fallback, so it honestly remains retryable. City checks are 5/9 and Ithaca remains active; no policy was weakened to force progress. Forty-seven targeted and 409 full unittest tests pass with zero failures/errors; there were no production writes, SMTP/IMAP, Inventory, scheduler or frozen-file changes, and no replay-owned browser orphan.
